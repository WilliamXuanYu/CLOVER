...
class MetricCacheLoader:
    ...
    @staticmethod
    def _normalize_metric_cache_path(path: str) -> str:
        """
        Normalize metric-cache file paths read from nuPlan metadata CSV.

        Optional prefix rewrite (opt-in via env): if metric-cache metadata was
        generated on another filesystem layout, set
        ``NAVSIM_METRIC_CACHE_PATH_SUBST_FROM`` and
        ``NAVSIM_METRIC_CACHE_PATH_SUBST_TO``. If ``FROM`` appears in ``path``,
        replace the first occurrence with ``TO``.
        """
        ...
        if len(path) >= 2 and path[0] == path[-1] == "'":
            path = path[1:-1].strip()
        # Metadata 可能来自含 JJ_Group 的挂载布局；与本机目录不一致时去掉该段。
        path = path.replace("JJ_Group/", "", 1)
    ...
    def _load_metric_cache_paths(self, cache_path: Path) -> Dict[str, Path]:
        ...
        metric_cache_dict = {
            p.split("/")[-2]: self._normalize_metric_cache_path(p)
