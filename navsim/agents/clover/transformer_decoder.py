...
class Attention(torch.nn.Module):
    ...
    def __init__(
        ...
        self.attn = torch.nn.MultiheadAttention(dim, num_heads, 
                                                dropout=0.0, bias=True, add_bias_kv=False, add_zero_attn=False, kdim=self.kv_dim, vdim=self.kv_dim, batch_first=True, device=None, dtype=None)
...
class Block(torch.nn.Module):
    ...
    def __init__(self,
        ...
        kv_dim = cross_kv_dim if cross_kv_dim is not None else dim
        self.cross_attn_norm_kv = norm_layer(kv_dim)
...
class TransformerDecoderScorer(torch.nn.Module):
    ...
    def __init__(self, num_layers, d_model, proj_drop, drop_path, config, cross_kv_dim: Optional[int] = None):
        super().__init__()
