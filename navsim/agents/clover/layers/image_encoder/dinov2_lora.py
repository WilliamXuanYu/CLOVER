...
def _is_pytorch_lightning_checkpoint_path(path: str) -> bool:
    if not path:
        return False
    p = str(path).lower()
    return p.endswith(".ckpt") or p.endswith(".pth")


def _load_pl_checkpoint_raw(path: str) -> dict:
    """Load a PyTorch Lightning or generic .ckpt; return a flat state_dict."""
    try:
...
def _strip_longest_prefix(sd: dict, allowed_prefixes):
    """Use the first prefix in allowed_prefixes that matches any key; return (prefix, stripped_dict)."""
    items = list(sd.items())
    for prefix in allowed_prefixes:
        out = {k[len(prefix) :]: v for k, v in items if k.startswith(prefix)}
        if out:
            return prefix, out
    return None, {}


def _image_backbone_state_from_checkpoint(model_weights: str) -> dict:
    """Load ImgEncoder state dict from a full Lightning checkpoint (or a raw backbone dict)."""
    full_sd = _load_pl_checkpoint_raw(model_weights)
    # Order: DrivoR training checkpoint (NAVSIM agent wrapper) first, then plain module keys.
    candidates = [
        "agent._drivor_model.image_backbone.",
        "image_backbone.",
        "module.agent._drivor_model.image_backbone.",
        "module.image_backbone.",
    ]
    prefix, sub = _strip_longest_prefix(full_sd, candidates)
    if not sub:
        # Single-module export: all keys are already for ImgEncoder (model.*, neck.*, ...).
        sample_key = next(iter(full_sd), "")
        if sample_key and (
            sample_key.startswith("model.")
            or sample_key.startswith("neck.")
        ):
            return dict(full_sd)
        raise RuntimeError(
            f"No image_backbone.* keys in checkpoint {model_weights!r}. "
            f"Tried prefixes: {candidates!s}. First keys: {list(full_sd)[:5]!s}"
        )
    log.info(
        "Loaded image_backbone weights from PL checkpoint %s (prefix %r, %d tensors).",
        model_weights,
        prefix,
        len(sub),
    )
...
def _map_hf_dinov3_to_timm_vit(hf_state_dict):
    """Map HF DINOv3 checkpoint keys to timm VisionTransformer keys."""
    mapped = {}
    if "embeddings.cls_token" in hf_state_dict:
        mapped["cls_token"] = hf_state_dict["embeddings.cls_token"]
    if "embeddings.patch_embeddings.weight" in hf_state_dict:
        mapped["patch_embed.proj.weight"] = hf_state_dict["embeddings.patch_embeddings.weight"]
    if "embeddings.patch_embeddings.bias" in hf_state_dict:
        mapped["patch_embed.proj.bias"] = hf_state_dict["embeddings.patch_embeddings.bias"]
    if "norm.weight" in hf_state_dict:
        mapped["norm.weight"] = hf_state_dict["norm.weight"]
    if "norm.bias" in hf_state_dict:
        mapped["norm.bias"] = hf_state_dict["norm.bias"]

    layer_idx = 0
    while f"layer.{layer_idx}.norm1.weight" in hf_state_dict:
        h = f"layer.{layer_idx}."
        t = f"blocks.{layer_idx}."
        q_w = hf_state_dict[h + "attention.q_proj.weight"]
        k_w = hf_state_dict[h + "attention.k_proj.weight"]
        v_w = hf_state_dict[h + "attention.v_proj.weight"]
        mapped[t + "attn.qkv.weight"] = torch.cat([q_w, k_w, v_w], dim=0)

        q_b = hf_state_dict[h + "attention.q_proj.bias"]
        k_b = hf_state_dict.get(h + "attention.k_proj.bias", torch.zeros_like(q_b))
        v_b = hf_state_dict[h + "attention.v_proj.bias"]
        mapped[t + "attn.qkv.bias"] = torch.cat([q_b, k_b, v_b], dim=0)

        mapped[t + "attn.proj.weight"] = hf_state_dict[h + "attention.o_proj.weight"]
        mapped[t + "attn.proj.bias"] = hf_state_dict[h + "attention.o_proj.bias"]
        mapped[t + "mlp.fc1.weight"] = hf_state_dict[h + "mlp.up_proj.weight"]
        mapped[t + "mlp.fc1.bias"] = hf_state_dict[h + "mlp.up_proj.bias"]
        mapped[t + "mlp.fc2.weight"] = hf_state_dict[h + "mlp.down_proj.weight"]
        mapped[t + "mlp.fc2.bias"] = hf_state_dict[h + "mlp.down_proj.bias"]
        mapped[t + "norm1.weight"] = hf_state_dict[h + "norm1.weight"]
        mapped[t + "norm1.bias"] = hf_state_dict[h + "norm1.bias"]
        mapped[t + "norm2.weight"] = hf_state_dict[h + "norm2.weight"]
        mapped[t + "norm2.bias"] = hf_state_dict[h + "norm2.bias"]
        layer_idx += 1
    return mapped


def _maybe_build_dinov3_compat_model(model_name: str, model_weights: str, img_size, in_chans: int):
    """Build a timm ViT and load local DINOv3 safetensors in HF-format.

    This keeps compatibility with timm==0.9.x where DINOv3 model names are not registered.
    """
    dinov3_alias = {
        "timm/vit_small_patch16_dinov3.lvd1689m": "vit_small_patch16_224",
        "timm/vit_base_patch16_dinov3.lvd1689m": "vit_base_patch16_224",
        "facebook/dinov3-vits16-pretrain-lvd1689m": "vit_small_patch16_224",
        "facebook/dinov3-vitb16-pretrain-lvd1689m": "vit_base_patch16_224",
    }
    if model_name not in dinov3_alias:
        return None

    arch = dinov3_alias[model_name]
    ...
    if _is_pytorch_lightning_checkpoint_path(model_weights):
        # Weights for this run come from a Lightning .ckpt (image_backbone.*) after LoRA/ImgEncoder
        # is built; do not use safetensors here.
        log.info(
            "DINOv3 compat: skipping HF safetensors for %s; backbone will load from PL checkpoint: %s",
            arch,
            model_weights,
        )
        return model

    hf_state = load_file(model_weights)
    mapped_state = _map_hf_dinov3_to_timm_vit(hf_state)
    load_result = model.load_state_dict(mapped_state, strict=False)
    missing = [k for k in load_result.missing_keys if k != "pos_embed"]
    if missing:
        raise RuntimeError(
            f"DINOv3 compat load has unexpected missing keys: {missing[:20]}"
        )
    if load_result.unexpected_keys:
        raise RuntimeError(
            f"DINOv3 compat load has unexpected keys: {load_result.unexpected_keys[:20]}"
        )
    log.info(
        "Loaded DINOv3 HF safetensors into %s via compat path (missing allowed: pos_embed only).",
        arch,
    )
    return model
...
class ImgEncoder(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        pl_resume = _is_pytorch_lightning_checkpoint_path(config.model_weights)
        ...
        compat_model = _maybe_build_dinov3_compat_model(
        ...
        if pl_resume:
            sub = _image_backbone_state_from_checkpoint(config.model_weights)
            miss, unexp = self.load_state_dict(sub, strict=False)
            if miss or unexp:
                log.info(
                    "ImgEncoder PL resume: missing %d, unexpected %d (e.g. missing %s, unexpected %s)",
                    len(miss),
                    len(unexp),
                    list(miss)[:5],
                    list(unexp)[:5],
