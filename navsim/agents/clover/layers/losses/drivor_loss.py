...
class DrivoRLoss(torch.nn.Module):
    ...
    def __init__(self,
                        ...
                        pseudo_expert_weight: float = 0.5,
                        pseudo_expert_all_refinement: bool = False,
                        **kwargs):
        super().__init__()
        self.pseudo_expert_weight = pseudo_expert_weight
        self.pseudo_expert_all_refinement = pseudo_expert_all_refinement
    ...
    def multi_expert_coverage_loss(self, proposals, pseudo_experts, pseudo_expert_mask):
        """
        Coverage loss: for each pseudo-expert, find the closest proposal.
        Encourages generator to dedicate proposals to cover each expert mode.

        Args:
            proposals: [B, N, 8, 3] - N trajectory proposals
            pseudo_experts: [B, K, 8, 3] - K pseudo-expert trajectories (padded)
            pseudo_expert_mask: [B, K] - True where expert is valid
        Returns:
            scalar loss
        """
        # [B, N, K, 8, 3]
        diff = proposals[:, :, None] - pseudo_experts[:, None, :, :]
        # [B, N, K] mean L1 over poses and state dims
        dist = torch.linalg.norm(diff, dim=-1, ord=1).mean(dim=-1)
        # [B, K] for each expert, find closest proposal
        per_expert_min = dist.amin(dim=1)
        # mask out invalid experts
        mask_f = pseudo_expert_mask.float()
        valid_count = mask_f.sum(dim=1).clamp(min=1.0)
        loss = (per_expert_min * mask_f).sum(dim=1) / valid_count
        return loss.mean()
    ...
    def forward(self,targets: Dict[str, torch.Tensor], pred: Dict[str, torch.Tensor], config  , scoring_function=None):
        ...
        for proposals_i in proposal_list:
            ...
            if "pseudo_experts" in targets and self.pseudo_expert_weight > 0 and self.pseudo_expert_all_refinement:
                pe = targets["pseudo_experts"]
                pe_mask = targets["pseudo_expert_mask"]
                if pe_mask.any():
                    trajectory_loss = trajectory_loss + self.pseudo_expert_weight * self.multi_expert_coverage_loss(
                        proposals_i, pe, pe_mask
                    )

            min_loss_list.append(min_loss)
            inter_loss_list.append(inter_loss)

        ### MULTI-EXPERT COVERAGE LOSS
        me_loss = torch.tensor(0.0, device=proposals.device)
        if "pseudo_experts" in targets and self.pseudo_expert_weight > 0:
            pe = targets["pseudo_experts"]
            pe_mask = targets["pseudo_expert_mask"]
            if pe_mask.any():
                me_loss = self.multi_expert_coverage_loss(proposals, pe, pe_mask)
                # Default/original behavior: only add pseudo-expert loss on the final proposals.
                # Optional mode: pseudo_expert_all_refinement already added coverage inside the loop.
                if not self.pseudo_expert_all_refinement:
                    trajectory_loss = trajectory_loss + self.pseudo_expert_weight * me_loss
