...
_MULTI_NO_COLLISION = _multi_metric_member("NO_COLLISION")
_MULTI_DRIVABLE_AREA = _multi_metric_member("DRIVABLE_AREA")
_MULTI_TRAFFIC_LIGHT = _multi_metric_member("TRAFFIC_LIGHT_COMPLIANCE")
...
class PDMScorerConfig:
    ...
    def weighted_metrics_array(self) -> npt.NDArray[np.float64]:
        ...
        if _WEIGHTED_PROGRESS is not None:
            weighted_metrics[_WEIGHTED_PROGRESS] = self.progress_weight
        if _WEIGHTED_TTC is not None:
            weighted_metrics[_WEIGHTED_TTC] = self.ttc_weight
        if _WEIGHTED_COMFORT is not None:
            weighted_metrics[_WEIGHTED_COMFORT] = self.comfortable_weight
        if _WEIGHTED_DRIVING_DIRECTION is not None:
            weighted_metrics[_WEIGHTED_DRIVING_DIRECTION] = self.driving_direction_weight
...
class PDMScorer:
    ...
    def _aggregate_scores(self) -> npt.NDArray[np.float64]:
        ...
        self._weighted_metrics[_WEIGHTED_PROGRESS] = normalized_progress
    ...
    def _reset(
        ...
        if _MULTI_TRAFFIC_LIGHT is not None:
            self._multi_metrics[_MULTI_TRAFFIC_LIGHT] = 1.0
        if _MULTI_DRIVING_DIRECTION is not None:
            self._multi_metrics[_MULTI_DRIVING_DIRECTION] = 1.0
    ...
    def _calculate_no_at_fault_collision(self) -> None:
        ...
        self._multi_metrics[_MULTI_NO_COLLISION] = no_collision_scores
    ...
    def _calculate_drivable_area_compliance(self) -> None:
        ...
        self._multi_metrics[_MULTI_DRIVABLE_AREA] = drivable_area_compliance_scores
    ...
    def _calculate_driving_direction_compliance(self) -> None:
        ...
        if _WEIGHTED_DRIVING_DIRECTION is not None:
            self._weighted_metrics[_WEIGHTED_DRIVING_DIRECTION] = driving_direction_compliance_scores
        if _MULTI_DRIVING_DIRECTION is not None:
            self._multi_metrics[_MULTI_DRIVING_DIRECTION] = driving_direction_compliance_scores
    ...
    def _calculate_ttc(self):
        ...
        observation_horizon = len(getattr(self._observation, "_global_to_local_idcs", []))
        for time_idx in range(self.proposal_sampling.num_poses + 1):
            for step_idx, future_time_idx in enumerate(future_time_idcs):
                current_time_idx = time_idx + future_time_idx
                if current_time_idx >= observation_horizon:
                    continue
        ...
        self._weighted_metrics[_WEIGHTED_TTC] = ttc_scores
    ...
    def _calculate_is_comfortable(self) -> None:
        ...
        if _WEIGHTED_COMFORT is not None:
            self._weighted_metrics[_WEIGHTED_COMFORT] = np.all(self.is_comfortable, axis=-1)
